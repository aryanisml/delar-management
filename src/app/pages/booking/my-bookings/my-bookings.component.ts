...
export class MyBookingsComponent implements OnInit, OnDestroy {
  ...
  canStartInspection(booking: any, payment: any) {
    return String(booking?.status ?? '').toLowerCase() === 'approved' && this.hasPaidPayment(payment);
  }
  ...
  async downloadPaymentConfirmation(booking: any) {
    ...
    link.href = url;
    link.download = `receipt-${this.quotationPdf.buildFileName(input.quoteReference)}`;
    link.click();
  ...
  private buildQuotationPdfInput(booking: any, quotation: any): QuotationPdfInput | null {
    ...
    return {
      ...
      paymentSummaryLabel: this.paymentBadgeLabel(this.selectedPayment() || booking.latest_payment),
      paymentSummaryLines: this.paymentDetailLines(this.selectedPayment() || booking.latest_payment),
